...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'ยืนยันอีเมล',
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: Colors.black), // Minimal color scheme
        useMaterial3: true,
        fontFamily: 'SF Pro Display', // Example: If you want to use a custom font like SF Pro
                                     // You'll need to add it to pubspec.yaml and assets
        appBarTheme: const AppBarTheme(
          backgroundColor: Color.fromARGB(255, 255, 255, 255), // White app bar background
        ...
        inputDecorationTheme: InputDecorationTheme(
          ...
          errorBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(8.0),
            borderSide: BorderSide(color: Color.fromARGB(255, 0, 0, 0), width: 1.0), // Red border on error
          ),
          focusedErrorBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(8.0),
            borderSide: BorderSide(color: Colors.red, width: 2.0), // Red border on focused error
          ),
...
class _EmailVerificationPageState extends State<EmailVerificationPage> {
  ...
  // Hardcoded reference code as in the image
  final String _referenceCode = 'XYLP';
  final String _email = 'register@gmail.com'; // Example email from the image
  ...
  void _verifyOtp() {
    ...
    //     SnackBar(content: Text('กำลังตรวจสอบรหัส: $otp')),
    //   );
    // }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 16),
            Text(
              'กรอกรหัสที่ระบบส่งไปที่ $_email',
                ...
                return SizedBox(
                  ...
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      // You can add a hint text here if needed
                    ),
            ...
            Center(
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'รหัสมีผล 2 นาที ก่อนหมดอายุ', // The image says 5 mins but timer is 2 mins, adjust as needed
                    style: Theme.of(context).textTheme.bodyMedium,
                  ),
                  ...
                  GestureDetector(
                    onTap: _start == 0
                        ? () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('ส่งรหัสใหม่แล้ว!')),
                            );
                          }
                        : null, // Disable tap if timer is running
                    child: Text(
                      _start == 0 ? 'ส่งรหัสใหม่' : 'ส่งรหัสใหม่ครั้งถัดไปใน $_timerText',
                      style: Theme.of(context).textTheme.labelLarge?.copyWith(
                            color: _start == 0 ? Theme.of(context).colorScheme.primary : Colors.grey,
                          ),
                    ),
                  ),
                ],
