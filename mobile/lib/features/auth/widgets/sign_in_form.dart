...
import 'package:community_marketplace/features/auth/providers/sign_in_provider.dart';
...
class SignInForm extends StatelessWidget {
  ...
  void _handleEmailSignIn(BuildContext context, SignInProvider provider) async {
    try {
  ...
  void _handleEmailCheck(BuildContext context, SignInProvider provider) async {
    try {
      ...
      if (context.mounted) {
        if (!emailExists) {
          ...
          if (context.mounted) {
            // If register was successful, reset the form for SignIn
            if (result == true) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Center(
  ...
  Widget _buildActionButton(BuildContext context, SignInProvider provider) {
    return Selector<
      SignInProvider,
      ({
        bool canCheckEmail,
        ...
        if (!state.emailChecked) {
          ...
          return ConfirmButton(
            ...
            onPressed: state.canSignIn
                ? () => _handleEmailSignIn(context, provider)
                : () {},
